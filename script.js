...
function loadCSV() {
    ...
    fetch('https://raw.githubusercontent.com/username/repository/main/data.csv')
        .then(response => {
        ...
        .then(data => {
            ...
            mediaData = parseCSV(data); // Cần hàm parseCSV
...
function renderMedia() {
    ...
    mediaData.forEach((item, index) => {
        ...
        thumbnail.innerHTML = `<img src="${item.thumbnail}" alt="${item.title}" style="width:100px;height:100px;">`;
...
function playVideo(url, title, id, index) {
    ...
    video.onpause = () => {
        if (video.currentTime > 0) {
            localStorage.setItem(`videoTime_${ dair`, video.currentTime);
        }
    };
